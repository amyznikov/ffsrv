...
static struct http_server_ctx * create_http_server_ctx(const struct sockaddr_in * addrs, SSL_CTX * ssl_ctx);
...
static void http_client_destroy(struct http_client_ctx * client_ctx);
...
static struct http_client_ctx * create_http_client_ctx(int so, SSL_CTX * ssl_ctx)
{
  ...
  if ( !ssl_ctx ) {
    cosocket_set_rcvtmout(client_ctx->cosock, ffms.http.rcvtmo);
    cosocket_set_sndtmout(client_ctx->cosock, ffms.http.sndtmo);
  }
  else if ( !(client_ctx->ssl = ffms_ssl_new(ssl_ctx, client_ctx->cosock)) ) {
    PDBG("[so=%d] ffms_ssl_new() fails", so);
    goto end;
  }
  else {
    cosocket_set_rcvtmout(client_ctx->cosock, ffms.https.rcvtmo);
    cosocket_set_sndtmout(client_ctx->cosock, ffms.https.sndtmo);
  }
...
static void http_client_thread(void * arg)
{
  ...
  else if ( client_ctx->input ) {
    ff_run_input_stream(client_ctx->input);
  }

end:

  http_client_destroy(client_ctx);
...
bool ffms_add_http_port(const struct sockaddr_in * addrs)
{
  return create_http_server_ctx(addrs, NULL) != NULL;
}

bool ffms_add_https_port(const struct sockaddr_in * addrs)
{
  SSL_CTX * ssl_ctx = NULL;
  bool fok = false;

  if ( !(ssl_ctx = ffms_get_ssl_context()) ) {
    PDBG("ffms_create_ssl_context() fails");
    PSSL();
    goto end;
  }

  if ( !create_http_server_ctx(addrs, ssl_ctx) ) {
