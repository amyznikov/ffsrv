...
bool ffms_txtfile_setup(void)
{
  ...
  if ( ffms.db.txtfile.name && *ffms.db.txtfile.name ) {
    return access(ffms.db.txtfile.name, F_OK) == 0;
  }

  if ( !fok ) {
    snprintf(default_filename, sizeof(default_filename) - 1, "./ffms.db");
    fok = access(default_filename, F_OK) == 0;
  }

  if ( (uid = geteuid()) != 0 && (pw = getpwuid(uid)) != NULL ) {
    snprintf(default_filename, sizeof(default_filename) - 1, "%s/.config/ffms/ffms.db", pw->pw_dir);
    fok = access(default_filename, F_OK) == 0;
  }

  if ( !fok ) {
    snprintf(default_filename, sizeof(default_filename) - 1, "/var/lib/ffms/ffms.db");
    fok = access(default_filename, F_OK) == 0;
  }

  if ( !fok ) {
    snprintf(default_filename, sizeof(default_filename) - 1, "/etc/ffms.db");
    fok = access(default_filename, F_OK) == 0;
  }

  if ( !fok ) {
    PDBG("Can not access ffms.db");
  }
  else {
    free(ffms.db.txtfile.name);
    ffms.db.txtfile.name = strdup(default_filename);
  }
...
bool ffms_txtfile_find_object(const char * name, enum ffobject_type * __type, ffobj_params * params)
{
  FILE * fp = NULL;
  ...
  if ( !(fp = fopen(ffms.db.txtfile.name, "r")) ) {
    PDBG("fopen('%s') fails", ffms.db.txtfile.name);
    goto end;
  }
  ...
  if ( found && !fok ) {
    ffms_cleanup_object_params(type, params);
