...
static void destroy_http_server_ctx(struct http_server_ctx * server_ctx)
{
  if ( server_ctx ) {
    ...
    //ffsrv_destroy_ssl_context(&server_ctx->ssl_ctx);
    free(server_ctx);
...
static void http_client_destroy(struct http_client_ctx * client_ctx)
{
  if ( client_ctx ) {
    ...
    release_input(&client_ctx->input);
...
static bool on_http_method_post(struct http_client_ctx * client_ctx)
{
  ...
  status = create_input(&client_ctx->input, name,
...
bool ffsrv_add_https_port(const struct sockaddr_in * addrs)
{
  ...
  if ( !create_http_server_ctx(addrs, g_ssl_ctx) ) {
